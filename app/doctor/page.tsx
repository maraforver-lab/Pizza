  ...
  return (
    <main className="min-h-screen px-4 py-5 sm:px-6 sm:py-8">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="py-9 sm:py-12"><p className="text-xs font-extrabold uppercase tracking-[.2em] text-tomato">{t.eyebrow}</p><h1 className="mt-3 max-w-3xl font-display text-4xl font-semibold leading-none sm:text-6xl">{t.title}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-ink/55 sm:text-base">{t.intro}</p><div className="mt-6 max-w-3xl rounded-2xl border border-tomato/15 bg-tomato/[.06] p-4 sm:flex sm:items-center sm:justify-between sm:gap-5"><div><strong className="text-sm text-tomato">{t.verify}</strong><p className="mt-1 text-xs leading-5 text-ink/55">{t.verifyBody}</p></div><Link href={`/?${query}#recipe-settings`} className="mt-3 block shrink-0 rounded-xl bg-white px-4 py-3 text-center text-xs font-extrabold text-ink shadow-sm sm:mt-0">{t.edit} →</Link></div><div className="mt-3 inline-flex flex-wrap gap-x-3 gap-y-1 rounded-2xl bg-leaf/[.08] px-4 py-3 text-xs text-ink/55"><strong className="text-leaf">{t.recipe}:</strong><span className="font-bold text-ink">{pizzaStyle.nameEn}</span><span>{flour.brand} {flour.name}</span><span>{settings.hydration} %</span><span>{settings.fermentation.replaceAll("-", " ")}</span></div></section>
