...
const defaults: RecipeSettings = { pizzas: 4, ballWeight: 270, waste: 3, hydration: 65, salt: 2.8, yeastType: "idy", fermentation: "24h-cold", temperature: 4, goal: "balanced", ovenType: "home", flourId: "caputo-pizzeria" };

const copy = {
  fi: {
    calculator: "Laskuri", planner: "Aikataulu", doctor: "Taikinalääkäri", guide: "Ohjeet & termit", eyebrow: "Dough Doctor", title: "Miltä taikinasi näyttää?", intro: "Valitse kuva, joka muistuttaa omaa taikinaasi. Diagnoosi käyttää nykyisen reseptisi jauhoa, hydraatiota ja kohotusta.", recipe: "Diagnoosissa käytettävä resepti", choose: "Valitse lähin tilanne", likely: "Todennäköisin selitys", now: "Mitä voit tehdä nyt", next: "Muuta seuraavalla kerralla", edit: "Muokkaa reseptiä", plan: "Avaa aikataulu", note: "Kuvat ja diagnoosit ovat käytännöllisiä vertailuja. Taikinan todelliseen käytökseen vaikuttavat myös lämpötila, käsittely ja jauhoerä.", prompt: "Valitse ensin taikinaasi parhaiten vastaava kuva.",
  },
  en: {
    calculator: "Calculator", planner: "Planner", doctor: "Dough Doctor", guide: "Guide & glossary", eyebrow: "Dough Doctor", title: "What does your dough look like?", intro: "Choose the photo that resembles your dough. The diagnosis uses the flour, hydration and fermentation from your current recipe.", recipe: "Recipe used for diagnosis", choose: "Choose the closest situation", likely: "Most likely explanation", now: "What you can do now", next: "Change next time", edit: "Edit recipe", plan: "Open schedule", note: "The photos and diagnoses are practical comparisons. Actual dough behaviour also depends on temperature, handling, and flour batch.", prompt: "First choose the photo that best matches your dough.",
  },
} as const;
    ...
    <main className="min-h-screen px-4 py-5 sm:px-6 sm:py-8">
      <div className="mx-auto max-w-6xl">
        <header className="flex items-center justify-between gap-4">
          ...
          <nav className="hidden items-center gap-1 rounded-full border border-ink/10 bg-white/70 p-1 lg:flex"><Link href={`/?${query}`} className="rounded-full px-4 py-2 text-xs font-bold text-ink/55">{t.calculator}</Link><Link href={`/plan?${query}`} className="rounded-full px-4 py-2 text-xs font-bold text-ink/55">{t.planner}</Link><span className="rounded-full bg-ink px-4 py-2 text-xs font-bold text-white">{t.doctor}</span></nav>
        ...
        <nav className="mt-4 flex gap-2 overflow-x-auto pb-1 lg:hidden"><Link href={`/?${query}`} className="shrink-0 rounded-full border border-ink/10 bg-white px-4 py-2 text-xs font-bold">{t.calculator}</Link><Link href={`/plan?${query}`} className="shrink-0 rounded-full border border-ink/10 bg-white px-4 py-2 text-xs font-bold">{t.planner}</Link><span className="shrink-0 rounded-full bg-ink px-4 py-2 text-xs font-bold text-white">{t.doctor}</span></nav>

        <section className="py-9 sm:py-12"><p className="text-xs font-extrabold uppercase tracking-[.2em] text-tomato">{t.eyebrow}</p><h1 className="mt-3 max-w-3xl font-display text-4xl font-semibold leading-none sm:text-6xl">{t.title}</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-ink/55 sm:text-base">{t.intro}</p><div className="mt-5 inline-flex flex-wrap gap-x-3 gap-y-1 rounded-2xl bg-leaf/[.08] px-4 py-3 text-xs text-ink/55"><strong className="text-leaf">{t.recipe}:</strong><span>{flour.brand} {flour.name}</span><span>{settings.hydration} %</span><span>{settings.fermentation.replaceAll("-", " ")}</span></div></section>

        <section><h2 className="font-display text-3xl font-semibold">{t.choose}</h2><div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3">{doctorIssues.map((issue) => { const labels = issueCopy[locale][issue.id]; const active = selected === issue.id; return <button key={issue.id} type="button" onClick={() => setSelected(issue.id)} aria-pressed={active} className={`group overflow-hidden rounded-2xl border text-left shadow-card transition active:scale-[.99] ${active ? "border-tomato bg-tomato text-white ring-4 ring-tomato/10" : "border-white bg-white"}`}><div className="relative aspect-square overflow-hidden bg-ink/5"><Image src={issue.image} alt={labels[0]} fill sizes="(max-width: 768px) 50vw, 33vw" className="object-cover transition duration-300 group-hover:scale-[1.03]"/></div><div className="p-3 sm:p-4"><strong className="block text-sm sm:text-base">{labels[0]}</strong><span className={`mt-1 hidden text-[11px] leading-4 sm:block ${active ? "text-white/65" : "text-ink/45"}`}>{labels[1]}</span></div></button>; })}</div></section>

        <section className="mt-7 overflow-hidden rounded-[1.75rem] bg-ink text-white shadow-card" aria-live="polite">{diagnosis ? <><div className="grid gap-5 p-5 sm:p-7 md:grid-cols-3"><article><span className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[#e8c98a]">01 · {t.likely}</span><p className="mt-3 text-sm leading-6 text-white/65">{diagnosis.cause}</p></article><article className="border-t border-white/10 pt-5 md:border-l md:border-t-0 md:pl-5 md:pt-0"><span className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[#e8c98a]">02 · {t.now}</span><p className="mt-3 text-sm leading-6 text-white/65">{diagnosis.now}</p></article><article className="border-t border-white/10 pt-5 md:border-l md:border-t-0 md:pl-5 md:pt-0"><span className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[#e8c98a]">03 · {t.next}</span><p className="mt-3 text-sm leading-6 text-white/65">{diagnosis.next}</p></article></div><div className="grid grid-cols-2 gap-2 border-t border-white/10 bg-white/[.04] p-4 sm:flex sm:justify-end"><Link href={`/?${query}`} className="rounded-xl border border-white/15 px-4 py-3 text-center text-xs font-bold text-white/70">{t.edit}</Link><Link href={`/plan?${query}`} className="rounded-xl bg-tomato px-4 py-3 text-center text-xs font-extrabold text-white">{t.plan}</Link></div></> : <p className="p-7 text-center text-sm text-white/50">{t.prompt}</p>}</section>
