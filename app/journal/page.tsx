...
import { BAKE_RESULTS_LOCAL_ONLY_COPY, deleteLocalBakeResult, loadLocalBakeResults } from "@/lib/local-bake-results";
import type { BakeResult } from "@/lib/bake-result";
...
export default function JournalPage() {
  ...
  const [savedBakes, setSavedBakes] = useState<BakeResult[]>([]);
  ...
  useEffect(() => {
    ...
    setSettings({ ...defaults, ...validShared }); document.documentElement.lang = "en";
    setSavedBakes(loadLocalBakeResults());
    loadJournalEntries().then(setEntries).catch(() => setNotice(copy.en.failed)).finally(() => setReady(true));
  }, []);
  ...
  const removeSavedBake = (id: string) => {
    if (!window.confirm(t.deleteBakeConfirm)) return;
    setSavedBakes(deleteLocalBakeResult(id));
  };
  ...
  return <main className="min-h-screen px-4 py-5 sm:px-6 sm:py-8"><div className="mx-auto max-w-6xl">
    ...
    <nav className="legacy-tool-nav mt-4 flex gap-2 overflow-x-auto pb-1 xl:hidden"><Link href={`/?${query}`} className="shrink-0 rounded-full border border-ink/10 bg-white px-4 py-2 text-xs font-bold">{t.calculator}</Link><Link href={`/plan?${query}`} className="shrink-0 rounded-full border border-ink/10 bg-white px-4 py-2 text-xs font-bold">{t.planner}</Link><Link href={`/doctor?${query}`} className="shrink-0 rounded-full border border-ink/10 bg-white px-4 py-2 text-xs font-bold">{t.doctor}</Link><Link href="/styles" className="shrink-0 rounded-full border border-ink/10 bg-white px-4 py-2 text-xs font-bold">{t.styles}</Link><span className="shrink-0 rounded-full bg-ink px-4 py-2 text-xs font-bold text-white">{t.journal}</span></nav>
    ...
    <section>
      <div className="rounded-[1.5rem] border border-white bg-white/70 p-5 shadow-card">
        <div className="flex items-center justify-between gap-4"><div><h2 className="font-display text-3xl font-semibold">{t.savedBakes}</h2><p className="mt-1 text-xs leading-5 text-ink/45">{t.localOnly}</p></div><span className="grid h-8 min-w-8 place-items-center rounded-full bg-ink px-2 text-xs font-bold text-white">{savedBakes.length}</span></div>
        {savedBakes.length === 0 ? <p className="mt-5 rounded-2xl border border-dashed border-ink/15 bg-white/50 px-4 py-8 text-center text-sm text-ink/40">{t.savedBakesEmpty}</p> : <div className="mt-5 space-y-3">{savedBakes.map((bake) => { const snapshot = bake.recipeSnapshot; const bakeFlour = flourById(snapshot.flourId); return <article key={bake.id} className="rounded-2xl border border-ink/10 bg-white p-4"><div className="flex items-start justify-between gap-3"><div><span className="rounded-full bg-leaf/10 px-2.5 py-1 text-[10px] font-extrabold uppercase text-leaf">{t.private}</span><h3 className="mt-2 font-display text-2xl font-semibold">{snapshot.pizzas} × {snapshot.ballWeight} g · {snapshot.hydration} %</h3><p className="mt-1 text-xs text-ink/45">{new Intl.DateTimeFormat("en-GB", { dateStyle: "medium" }).format(new Date(bake.createdAt))}</p></div>{bake.resultSnapshot?.overallRating && <strong className="text-sm text-tomato">★ {bake.resultSnapshot.overallRating}/5</strong>}</div><div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-xs"><span className="text-ink/45">{t.flour}</span><strong className="truncate text-right">{bakeFlour.brand} {bakeFlour.name}</strong><span className="text-ink/45">{t.fermentation}</span><strong className="text-right">{snapshot.fermentationLabel}</strong>{bake.bakingSnapshot?.bakeTimeSeconds && <><span className="text-ink/45">{t.bakeTime}</span><strong className="text-right">{bake.bakingSnapshot.bakeTimeSeconds} s</strong></>}{bake.bakingSnapshot?.ovenTemperatureCelsius && <><span className="text-ink/45">{t.ovenTemperature}</span><strong className="text-right">{bake.bakingSnapshot.ovenTemperatureCelsius} °C</strong></>}</div>{bake.resultSnapshot?.resultNotes && <p className="mt-4 rounded-xl bg-cream p-3 text-sm leading-6 text-ink/60">{bake.resultSnapshot.resultNotes}</p>}<div className="mt-4 grid grid-cols-2 gap-2"><Link href={`/?${recipeParams({ pizzas: snapshot.pizzas, ballWeight: snapshot.ballWeight, waste: snapshot.waste, hydration: snapshot.hydration, salt: snapshot.salt, yeastType: snapshot.yeastType, fermentation: snapshot.fermentation, temperature: snapshot.temperature, goal: snapshot.goal, ovenType: snapshot.ovenType, flourId: snapshot.flourId, pizzaStyleId: snapshot.pizzaStyleId }).toString()}`} className="rounded-xl bg-ink px-3 py-3 text-center text-xs font-bold text-white">{t.openRecipe}</Link><button type="button" onClick={() => removeSavedBake(bake.id)} className="rounded-xl border border-tomato/20 px-3 py-3 text-xs font-bold text-tomato">{t.delete}</button></div></article>; })}</div>}
      </div>

      <div className="mt-6"><div className="flex items-center justify-between gap-4"><h2 className="font-display text-3xl font-semibold">{t.entries}</h2><span className="grid h-8 min-w-8 place-items-center rounded-full bg-ink px-2 text-xs font-bold text-white">{entries.length}</span></div>{entries.length === 0 ? <p className="mt-5 rounded-2xl border border-dashed border-ink/15 bg-white/50 px-4 py-8 text-center text-sm text-ink/40">{t.empty}</p> : <div className="mt-5 space-y-4">{entries.map((entry) => { const entryFlour = flourById(entry.settings.flourId); const isOpen = expanded === entry.id; return <article key={entry.id} className="overflow-hidden rounded-[1.5rem] border border-white bg-white/80 shadow-card"><div className="grid sm:grid-cols-[11rem_1fr]">{entry.photo ? <div className="relative aspect-video bg-ink/5 sm:aspect-auto sm:min-h-44"><JournalPhoto blob={entry.photo} alt={entry.name}/></div> : <div className="grid aspect-video place-items-center bg-leaf/[.08] text-4xl sm:aspect-auto">🍕</div>}<div className="p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-display text-2xl font-semibold">{entry.name}</h3><p className="mt-1 text-xs text-ink/40">{new Intl.DateTimeFormat("en-GB", { dateStyle: "long" }).format(new Date(`${entry.bakedAt}T12:00:00`))}</p></div><div className="text-right text-xs font-bold text-tomato">★ {entry.tasteRating}/5</div></div><p className="mt-3 text-xs text-ink/50">{entryFlour.brand} {entryFlour.name} · {entry.settings.hydration} % · {entry.settings.fermentation.replaceAll("-", " ")}</p><button type="button" onClick={() => setExpanded(isOpen ? null : entry.id)} className="mt-3 text-xs font-extrabold text-leaf">{isOpen ? t.hide : t.show} {isOpen ? "↑" : "↓"}</button></div></div>{isOpen && <div className="border-t border-ink/10 p-5"><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-xl bg-cream p-3"><span className="block text-[9px] uppercase text-ink/35">{t.taste}</span><strong>★ {entry.tasteRating}/5</strong></div><div className="rounded-xl bg-cream p-3"><span className="block text-[9px] uppercase text-ink/35">{t.texture}</span><strong>★ {entry.textureRating}/5</strong></div><div className="rounded-xl bg-cream p-3"><span className="block text-[9px] uppercase text-ink/35">{t.handling}</span><strong>★ {entry.handlingRating}/5</strong></div></div><p className="mt-4 text-sm leading-6 text-ink/60">{entry.notes || t.noNotes}</p>{entry.nextTime && <p className="mt-3 rounded-xl bg-tomato/[.06] p-3 text-sm leading-6 text-ink/60"><strong className="text-tomato">{t.nextTime}</strong><br/>{entry.nextTime}</p>}<div className="mt-4 grid grid-cols-2 gap-2"><Link href={`/?${recipeParams(entry.settings).toString()}`} className="rounded-xl bg-ink px-3 py-3 text-center text-xs font-bold text-white">{t.openRecipe}</Link><button type="button" onClick={() => removeEntry(entry.id)} className="rounded-xl border border-tomato/20 px-3 py-3 text-xs font-bold text-tomato">{t.delete}</button></div></div>}</article>; })}</div>}</div>
    </section></div>
