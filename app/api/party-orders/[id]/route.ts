...
export async function GET(
  ...
  if (error) return NextResponse.json({ error: "Party Order could not be loaded." }, { status: 500 });
  ...
  if (submissionsError) return NextResponse.json({ error: "Party Order activity could not be loaded." }, { status: 500 });
  ...
  if (submissionIds.length) {
    ...
    if (itemsError) return NextResponse.json({ error: "Party Order activity could not be loaded." }, { status: 500 });
...
export async function PATCH(
  ...
  if (existingError) return NextResponse.json({ error: "Party Order could not be loaded." }, { status: 500 });
  ...
  const detailKeys = ["title", "pizzaDateTime", "ordersCloseAt", "timeZone", "guestNote", "allowedPizzaIds", "pizza_datetime", "orders_close_at", "time_zone", "guest_note", "allowed_pizza_ids"];
  ...
  if (isStatusUpdate) {
    ...
    updateValues = {
      ...
      time_zone: validation.value.time_zone,
  ...
  if (error) return NextResponse.json({ error: "Party Order could not be updated." }, { status: 500 });
  const event = normalizePartyOrderRow(data);
