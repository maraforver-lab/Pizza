...
const copy = {
  fi: { back: "Laskuri", guide: "Ohjeet ja terminologia", eyebrow: "Cost Calculator", title: "Mitä pizzailta oikeasti maksaa?", intro: "Nykyisen taikinareseptin lisäksi mukaan lasketaan kastike, juusto, täytteet, energia, pakkaukset ja hävikki.", batch: "Erä ja taikina", pizzas: "Pizzoja", diners: "Syöjiä", ball: "Pallon paino", hydration: "Hydraatio", ingredients: "Raaka-aineiden hinnat", perKg: "€/kg", perPizza: "€/pizza", gramsPizza: "g/pizza", flour: "Jauhot", sauce: "Tomaattikastike", cheese: "Juusto", toppings: "Muut täytteet", extras: "Öljy, basilika ja mausteet", energy: "Uunin energia / paistokerta", packaging: "Laatikko tai pakkaus", waste: "Hävikki", results: "Kustannusyhteenveto", whole: "Koko pizzailta", one: "Yksi pizza", diner: "Yksi syöjä", doughExtras: "Vesi, suola ja hiiva", selling: "Myyntihinta (valinnainen)", sellingPrice: "Myyntihinta / pizza", revenue: "Liikevaihto", profit: "Tulos kulujen jälkeen", margin: "Kate", note: "Syötä omien pakkaustesi todelliset kilohinnat. Oletusarvot ovat vain käyttökelpoinen lähtökohta.", lineCost: "Hinta", amount: "Määrä", imported: "Taikinan koko luettiin nykyisestä reseptistä." },
  en: { back: "Calculator", guide: "Guide & glossary", eyebrow: "Cost Calculator", title: "What does pizza night really cost?", intro: "The current dough batch is combined with sauce, cheese, toppings, energy, packaging and waste.", batch: "Batch and dough", pizzas: "Pizzas", diners: "Diners", ball: "Ball weight", hydration: "Hydration", ingredients: "Ingredient prices", perKg: "€/kg", perPizza: "€/pizza", gramsPizza: "g/pizza", flour: "Flour", sauce: "Tomato sauce", cheese: "Cheese", toppings: "Other toppings", extras: "Oil, basil and seasoning", energy: "Oven energy / session", packaging: "Box or packaging", waste: "Waste", results: "Cost summary", whole: "Whole pizza night", one: "One pizza", diner: "One diner", doughExtras: "Water, salt and yeast", selling: "Selling price (optional)", sellingPrice: "Selling price / pizza", revenue: "Revenue", profit: "Result after costs", margin: "Margin", note: "Enter the real per-kilo prices from your packages. Defaults are only a useful starting point.", lineCost: "Cost", amount: "Amount", imported: "Dough size was read from the current recipe." },
} as const;

const currencyCopy = {
  fi: { title: "Valuutta", eur: "Euro (€)", usd: "Dollari ($)", note: "Syötä hinnat valitussa valuutassa. Valinta ei käytä vaihtokurssia." },
...
export default function CostsPage() {
  ...
  return <main className="min-h-screen bg-cream px-4 py-5 text-ink sm:px-6 sm:py-8"><div className="mx-auto max-w-6xl">
    <header className="flex items-center justify-between"><Link href="/" className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-tomato font-black text-white">{currencySymbol}</span><strong>Dough<span className="text-tomato">Tools</span></strong></Link><div className="flex gap-2"><Link href="/guide" className="hidden rounded-full border border-ink/10 bg-white px-4 py-2 text-xs font-bold sm:block">{t.guide}</Link><Link href="/" className="rounded-full bg-ink px-4 py-2 text-xs font-bold text-white">{t.back}</Link></div></header>
    <section className="py-10 sm:py-14"><p className="text-xs font-extrabold uppercase tracking-[.22em] text-tomato">{t.eyebrow}</p><h1 className="mt-3 max-w-4xl font-display text-4xl font-semibold leading-none sm:text-6xl">{t.title}</h1><p className="mt-5 max-w-2xl text-sm leading-6 text-ink/55 sm:text-base">{t.intro}</p><p className="mt-3 text-xs font-bold text-leaf">✓ {t.imported}</p><div className="mt-6 inline-flex flex-col gap-2 rounded-2xl border border-ink/10 bg-white/80 p-2 shadow-sm sm:flex-row sm:items-center"><span className="px-2 text-xs font-extrabold text-ink/55">{currencyText.title}</span><div className="flex rounded-xl bg-ink/5 p-1">{(["EUR", "USD"] as Currency[]).map(option => <button key={option} type="button" onClick={() => changeCurrency(option)} aria-pressed={currency === option} className={`rounded-lg px-4 py-2 text-xs font-extrabold transition ${currency === option ? "bg-ink text-white shadow-sm" : "text-ink/45"}`}>{option === "EUR" ? currencyText.eur : currencyText.usd}</button>)}</div><span className="max-w-xs px-2 text-[10px] leading-4 text-ink/40">{currencyText.note}</span></div></section>
