...
export default function TimerPage() {
  ...
  useEffect(() => () => { void releaseWakeLock(); audio.current?.close(); }, [releaseWakeLock]);
  ...
  return <main className={`min-h-screen px-4 py-6 pb-28 text-ink transition-colors duration-300 sm:px-6 ${urgent ? "bg-[#b62f20]" : "bg-cream"}`}><div className="mx-auto max-w-4xl">
    <div className="flex items-center justify-between gap-3"><Link href={`/plan${query}`} className={`rounded-full px-4 py-2 text-xs font-bold ${urgent ? "bg-white/15 text-white" : "bg-white text-ink/65"}`}>← {t.back}</Link><button type="button" onClick={() => setMuted(current => !current)} className={`rounded-full px-4 py-2 text-xs font-bold ${urgent ? "bg-white/15 text-white" : "bg-white text-ink/65"}`}>{muted ? "🔇 " + t.mute : "🔊 " + t.sound}</button></div>
    ...
    <section className="mt-5 rounded-[1.5rem] bg-white/80 p-5 shadow-card"><div className="flex gap-3"><span className="text-xl">{wakeStatus === "active" ? "☀" : "◐"}</span><div><strong className={`text-sm ${wakeColor}`}>{wakeText}</strong><p className="mt-1 text-xs leading-5 text-ink/45">{t.note}</p></div></div></section>
