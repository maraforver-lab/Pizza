...
describe("Pizza Session shopping list presets", () => {
  ...
  it("adds the /session/shopping route, secondary pizza mix UI and local-first copy without a free-text ingredient editor", () => {
    ...
    expect(page).toContain("Check ingredients, choose toppings, then continue your pizza plan.");
    expect(page).toContain("These are the ingredients you need to make the dough, make the sauce and choose toppings. Check what you already have and what you still need to buy.");
    ...
    expect(page).toContain("Choose your pizza");
    expect(page).toContain("Choose toppings");
    ...
    expect(page).toContain("Choose toppings");
    expect(page).toContain("Hide topping choices");
  ...
  it("renders the shopping page as a compact grouped checklist with icons and live progress", () => {
    ...
    expect(page).toContain("These are the ingredients you need to make the dough, make the sauce and choose toppings.");
    expect(page).toContain("Dough amounts come from the Dough Plan. Toppings follow your selected pizzas: {selectedPizzaMixSummary}.");
    expect(page).toContain('<p className="text-sm font-bold leading-6 text-ink/60 sm:hidden">');
    expect(page).toContain('<p className="hidden text-xs font-extrabold uppercase tracking-[.18em] text-leaf sm:block">Shopping list</p>');
    expect(page).toContain('<h2 id="shopping-checklist-heading" className="hidden font-display text-2xl font-semibold sm:mt-1 sm:block">Shopping list</h2>');
  ...
  it("adds one secondary contextual Toppings link after the checklist-first Shopping workflow", () => {
    ...
    expect(page.indexOf("Shopping list")).toBeLessThan(page.indexOf("<BottomActionBar"));
    ...
    expect(page.indexOf("Continue your pizza plan →")).toBeLessThan(page.indexOf('href="/toppings"'));
  ...
  it("aligns the shopping page with Pizza Session V2 checklist structure", () => {
    ...
    expect(page.indexOf("Shopping list")).toBeLessThan(page.indexOf("<BottomActionBar"));
    expect(page.indexOf("<BottomActionBar")).toBeLessThan(page.indexOf("Optional shopping tools"));
    expect(page.indexOf("Optional shopping tools")).toBeLessThan(page.indexOf("Hide topping choices"));
    expect(page.indexOf("Hide topping choices")).toBeLessThan(page.indexOf("PIZZA_MIX_OPTIONS.map"));
    ...
    expect(page).toContain('<p className="text-xs font-extrabold uppercase tracking-[.16em] text-tomato sm:hidden">Choose toppings</p>');
    expect(page).toContain('<h2 id="pizza-menu-controls-heading" className="mt-1 hidden font-display text-2xl font-semibold sm:block">Choose your pizza</h2>');
