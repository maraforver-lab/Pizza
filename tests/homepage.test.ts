...
describe("homepage content model", () => {
  ...
  it("keeps secondary discovery routes valid and unique", () => {
    ...
    expect(homepageContent.secondaryTools.map((tool) => tool.name).join("\n")).not.toContain("Pizza Coach");
    expect(homepageContent.coreTools.map((tool) => tool.href)).toContain("/guide/pizza-troubleshooting");
  ...
  it("renders the full Pizza Session landing page instead of old homepage clutter", () => {
    ...
    expect(homepage).toContain("lg:object-[58%_58%]");
    expect(homepage).toContain("xl:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)]");
  ...
  it("keeps the homepage footer concise with approved link groups and routes", () => {
    ...
    for (const expected of [
      ...
      'label: "Choose toppings", href: "/toppings"',
      'label: "Fix pizza problems", href: "/guide/pizza-troubleshooting"',
      'label: "Practical tips", href: "/guide/practical-pizza-tips"',
  ...
  it("opens the direct calculator route into a focused existing calculator entry view", () => {
    const homepage = source("app/page.tsx");
    const calculatorWorkspace = source("components/HomeCalculatorWorkspace.tsx");
  ...
  it("keeps Calculator v2 available directly while excluding it from global navigation", () => {
    const header = source("components/GlobalToolNavigation.tsx");
    const homepage = source("app/page.tsx");
    const calculatorWorkspace = source("components/HomeCalculatorWorkspace.tsx");
  ...
  it("keeps the homepage hero image, CTA and experience selector balanced responsively", () => {
    ...
    expect(homepage).toContain("overflow-x-clip");
    expect(homepage).toContain("lg:min-h-[clamp(34rem,calc(100svh-6rem),40rem)]");
    expect(homepage).toContain("xl:min-h-[clamp(35rem,calc(100svh-6rem),41rem)]");
    expect(homepage).toContain("getImageProps");
    expect(homepage).toContain("<picture>");
    expect(homepage).toContain('media="(min-width: 1024px)"');
    expect(homepage).toContain("lg:object-[58%_58%]");
    expect(homepage).toContain("xl:object-[58%_55%]");
    expect(homepage).toContain("lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]");
    expect(homepage).toContain("xl:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)]");
    expect(homepage).toContain("text-[clamp(2.5rem,11vw,5.8rem)]");
    expect(homepage).toContain("max-w-[9ch]");
    expect(homepage).toContain("lg:max-w-[13ch]");
    expect(homepage).toContain("xl:max-w-[14ch]");
