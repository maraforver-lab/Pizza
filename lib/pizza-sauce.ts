export type SauceStyle = "neapolitan" | "home" | "new-york";
...
export function calculateSauce(style: SauceStyle, pizzas: number, gramsPerPizza: number, waste: number) {
  ...
  const reduction = style === "neapolitan" ? 0 : style === "home" ? 0.15 : 0.18;
  const tomatoes = finished / (1 - reduction);
  const saltRate = style === "new-york" ? 0.012 : 0.01;
  const oilRate = style === "neapolitan" ? 0 : style === "home" ? 0.02 : 0.025;
  ...
  if (style !== "neapolitan") ingredients.push(
    ...
    { id: "garlic", amount: Math.max(1, Math.round(tomatoes / (style === "home" ? 600 : 450))), unit: "cloves" },
    { id: "oregano", amount: tomatoes * (style === "home" ? 0.0015 : 0.0025), unit: "g" },
  );
  if (style === "neapolitan") ingredients.push(
