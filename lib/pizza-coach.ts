...
export function buildCoachAdvice(locale: CoachLocale, settings: RecipeSettings, flour: FlourProfile, goal: CoachGoal, issue: CoachIssue) {
  ...
  if (issue === "pale") add("Vaalea pinta: lisää ylälämpöä", "Pale top: increase top heat", "Esilämmitä uuni ja kivi täysin. Kaasuuunissa nosta liekkiä juuri ennen pizzaa; sähköuunissa käytä grillivastusta hallitusti.", "Fully preheat oven and stone. In gas, raise the flame just before launch; at home, use the broiler carefully.", "now");
  ...
  return {
    summary: fi ? `Valmentajan lähtökohta: ${settings.pizzas} × ${settings.ballWeight} g, ${settings.hydration} % hydraatio, ${flourName} ja ${settings.ovenType === "gas" ? "kaasupizzauuni" : "sähköuuni"}.` : `Coach starting point: ${settings.pizzas} × ${settings.ballWeight} g, ${settings.hydration}% hydration, ${flourName}, and a ${settings.ovenType === "gas" ? "gas pizza oven" : "electric oven"}.`,
